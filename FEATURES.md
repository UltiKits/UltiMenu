...
| ultimenu.menu.submenu-open | A button whose `open-menu` key names another loaded menu closes the current inventory and opens the named menu on the next server tick, instead of running close-on-click/other click effects | gui | click a button configured with `open-menu` | n/a | n/a | player | brief | CustomMenuGui#handleButtonClick |
...
| ultimenu.config.menu-definition.permission | Permission required to open this menu at all, checked in `MenuCommands#openMenuByName` and `ItemBindListener#tryOpenMenuForItem` before the GUI is constructed; unset or empty means anyone with the base `ultikits.menu.use` permission may open it | config | `menus/<name>.yml: permission (default: unset — no per-menu check)` | n/a | n/a | admin | brief | MenuServiceImpl#parseMenuFile |
