...
| ultimenu.menu.submenu-open | A button whose `open-menu` key names another loaded menu closes the current inventory and opens the named menu on the next server tick; permission, economy and player/console commands configured on the SAME button still run first, in the fixed order this section's preamble states — only the trailing `close-on-click` branch is skipped in favor of the sub-menu navigation, not those earlier effects. This is a THIRD menu-access path alongside the command path (`MenuCommands#openMenuByName`) and the bound-item path (`ItemBindListener#tryOpenMenuForItem`), and it is the WEAKEST of the three: it checks neither `ultikits.menu.use` NOR the target sub-menu's own `permission` key at all — `CustomMenuGui#handleButtonClick`'s `open-menu` branch calls `menuService.getMenu(openMenu)` and opens it directly, with no permission check of any kind on the sub-menu being entered (`UltiKits/UltiMenu#15`, filed, not fixed here) | gui | click a button configured with `open-menu` | n/a | n/a | player | detailed | CustomMenuGui#handleButtonClick |
...
| ultimenu.config.menu-definition.buttons.permission | Permission node required to click this specific button; unset or empty means any viewer of the menu may click it. For a viewer who reached this menu via the COMMAND or BOUND-ITEM path, the menu-level `permission` key already gated who could open it at all — but for a viewer who reached it via ANOTHER menu's `open-menu` button, this menu's own `permission` key was NEVER checked at all (`UltiKits/UltiMenu#15`, see `ultimenu.menu.submenu-open`'s own row), so an unset button-level `permission` is NOT safe-by-default "defense in depth" for a menu reachable that way | config | `menus/<name>.yml: buttons.<id>.permission (default: unset — no button-level check)` | n/a | n/a | admin | detailed | MenuServiceImpl#parseButton |
